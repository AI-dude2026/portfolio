...
async function fetchGitHubContributions() {
    ...
    try {
        ...
        if (data.data && data.data.user) {
            renderGraphQLContributionGraph(data.data.user.contributionsCollection.contributionCalendar);
        } else {
            console.error('GraphQL error:', data);
            document.getElementById('total-contributions').textContent = 'Error loading contributions';
        }
    } catch (error) {
...
function renderGraphQLContributionGraph(calendar) {
    ...
    calendar.weeks.forEach(week => {
        ...
        week.contributionDays.forEach(day => {
            ...
            dayEl.title = `${day.contributionCount} contribution${day.contributionCount !== 1 ? 's' : ''} on ${date.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}`;
